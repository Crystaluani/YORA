...
export default function ProfileSetup() {
  ...
  const handleSave = async () => {
    if (!form.name || !form.genre) {
      alert("Please fill in your name and creative field")
      return
    }
    setLoading(true)
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        .ps { background: #080808; min-height: 100vh; color: #fff; font-family: 'DM Sans', sans-serif; padding: 60px 20px 80px; }
        ...
        .ps-input::placeholder { color: #333; }
        ...
        .ps-submit:hover:not(:disabled) { opacity: 0.85; }
        ...
        .field-option { padding: 12px 16px; border: 1px solid #1f1f1f; border-radius: 12px; font-size: 14px; color: #666; cursor: pointer; transition: all 0.2s; background: transparent; font-family: 'DM Sans', sans-serif; text-align: left; width: 100%; display: flex; align-items: center; gap: 10px; }
        .field-option:hover { border-color: #333; color: #aaa; }
        .field-option.selected { border-color: #d4af37; color: #d4af37; background: rgba(212,175,55,0.06); }
        .field-option-dot { width: 16px; height: 16px; border-radius: 50%; border: 1.5px solid currentColor; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        .field-option-dot-inner { width: 8px; height: 8px; border-radius: 50%; background: #d4af37; display: none; }
        .field-option.selected .field-option-dot-inner { display: block; }
        .soon-pill { display: inline-block; padding: 7px 14px; border: 1px solid #1a1a1a; border-radius: 999px; font-size: 12px; color: #333; margin: 4px; }
      ...
      <div className="ps">
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <h1 className="ps-syne" style={{ fontSize: 30, fontWeight: 800, margin: "0 0 8px", letterSpacing: "-0.02em" }}>
              Set up your profile
            </h1>
            <p style={{ color: "#555", fontSize: 15, margin: 0 }}>
              This is what clients and venues will see
            </p>
          </div>
            ...
            {step === 1 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div>
                  ...
                  <input className="ps-input" placeholder="Your name or stage name" value={form.name} onChange={e => set("name", e.target.value)} />
                </div>

                <div>
                  <label className="ps-label">Location</label>
                  <input className="ps-input" placeholder="Lagos, Nigeria" value={form.location} onChange={e => set("location", e.target.value)} />
                </div>

                <div>
                  <label className="ps-label">Bio</label>
                  <textarea className="ps-input" placeholder="Tell clients who you are and what you do..." rows={3} value={form.bio} onChange={e => set("bio", e.target.value)} style={{ resize: "vertical" }} />
                </div>

                <button className="ps-submit" onClick={() => { if (!form.name) return alert("Please enter your name"); setStep(2) }}>
                  Continue →
                </button>
              </div>
            )}

            {/* ── STEP 2: Creative Field ── */}
            {step === 2 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  <p className="ps-syne" style={{ fontSize: 18, fontWeight: 700, margin: "0 0 4px" }}>Your creative field</p>
                  <p style={{ color: "#555", fontSize: 13, margin: 0 }}>Select the category that best describes your work</p>
                </div>

                {/* Music subcategories */}
                <div style={{ background: "#080808", border: "1px solid #1a1a1a", borderRadius: 14, padding: "16px 16px 8px" }}>
                  <p style={{ fontSize: 11, color: "#d4af37", letterSpacing: "0.15em", textTransform: "uppercase", margin: "0 0 12px", fontWeight: 500 }}>
                    ✓ Musical Artist
                  </p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {MUSIC_TYPES.map(type => (
                      <button
                        key={type}
                        className={`field-option${form.genre === type ? " selected" : ""}`}
                        onClick={() => set("genre", type)}
                      >
                        <span className="field-option-dot">
                          <span className="field-option-dot-inner" />
                        </span>
                        {type}
                      </button>
                    ))}
                ...
                {/* Coming soon */}
                <div style={{ background: "#080808", border: "1px solid #1a1a1a", borderRadius: 14, padding: "16px", opacity: 0.6 }}>
                  <p style={{ fontSize: 11, color: "#555", letterSpacing: "0.15em", textTransform: "uppercase", margin: "0 0 12px", fontWeight: 500 }}>
                    Coming soon
                  </p>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 0 }}>
                    {COMING_SOON.map(type => (
                      <span key={type} className="soon-pill">{type}</span>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input className="ps-input" placeholder="e.g. Live performer, recording artist, wedding shows" value={form.skills} onChange={e => set("skills", e.target.value)} />
                  <p style={{ fontSize: 11, color: "#444", margin: "5px 0 0" }}>Separate with commas</p>
                </div>

                <button className="ps-submit" onClick={() => { if (!form.genre) return alert("Please select your creative field"); setStep(3) }}>
                  Continue →
                </button>
            ...
            {step === 3 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div>
                  ...
                  <p style={{ color: "#555", fontSize: 13, margin: 0 }}>Help clients know what to expect</p>
                </div>

                <div>
                  <label className="ps-label">Starting Price</label>
                  <input className="ps-input" placeholder="e.g. ₦150,000 or $500" value={form.price_range} onChange={e => set("price_range", e.target.value)} />
                  <p style={{ fontSize: 11, color: "#444", margin: "5px 0 0" }}>
                    Shown on your profile so clients know your range upfront
                  </p>
                </div>

                <div>
                  <label className="ps-label" style={{ marginBottom: 14 }}>Availability</label>
                  <div className="toggle" onClick={() => set("available", !form.available)}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 12, color: "#555", margin: 0 }}>
                        {form.available ? "Clients can send you booking requests" : "Profile visible but bookings paused"}
                      </p>
                    </div>
                  </div>
                </div>
