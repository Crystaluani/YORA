...
export default function ArtistProfilePage() {
  ...
  const saveBooking = async (paymentRef: string | null, paidAmount: number) => {
    ...
    await supabase.from("notifications").insert({
      user_id: artist.id,
      type: "booking",
      message: `New booking request from ${form.requester_name}${paidAmount > 0 ? ` · ₦${paidAmount.toLocaleString()} deposit paid` : ""}`,
      link: "/dashboard"
    })
  ...
  const handleBookingFree = async () => {
    if (!validateForm()) return
    setLoading(true)
    const ok = await saveBooking(null, 0)
    if (ok) setSubmitted(true)
    setLoading(false)
  }

  // Pay with Paystack then save
  const handlePayAndBook = () => {
    ...
    const handler = PaystackPop.setup({
      ...
      metadata: {
        ...
        booker_name: form.requester_name,
      },
      callback: async (response: any) => {
        ...
        try {
          ...
          if (verifyData.success) {
            const ok = await saveBooking(response.reference, amount)
            if (ok) setSubmitted(true)
          } else {
  ...
  if (notFound) return (
    <div style={{ background: "#080808", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ color: "#555", fontFamily: "'DM Sans',sans-serif" }}>Creative not found</p>
        <a href="/artists" style={{ color: "#d4af37", fontFamily: "'DM Sans',sans-serif", fontSize: 14, display: "block", marginTop: 16 }}>← Browse creatives</a>
      </div>
    </div>
  )
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .ap-share-btn { background: transparent; border: 1px solid #222; color: #666; font-family: 'DM Sans',sans-serif; font-size: 13px; padding: 10px 18px; border-radius: 999px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 6px; }
        ...
        .modal-bg { position: fixed; inset: 0; background: rgba(0,0,0,0.9); z-index: 200; display: flex; align-items: center; justify-content: center; padding: 16px; }
        ...
        .btn-pay { width: 100%; background: #d4af37; color: #000; font-weight: 700; font-family: 'DM Sans',sans-serif; font-size: 15px; padding: 14px; border-radius: 12px; border: none; cursor: pointer; transition: opacity 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; }
        .btn-pay:hover:not(:disabled) { opacity: 0.85; }
        .btn-pay:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-free { width: 100%; background: transparent; border: 1px solid #222; color: #666; font-family: 'DM Sans',sans-serif; font-size: 14px; padding: 12px; border-radius: 12px; cursor: pointer; transition: all 0.2s; margin-top: 10px; }
        .btn-free:hover { border-color: #444; color: #aaa; }
        .btn-cancel { width: 100%; background: transparent; border: 1px solid #1a1a1a; color: #444; font-family: 'DM Sans',sans-serif; font-size: 14px; padding: 12px; border-radius: 12px; cursor: pointer; margin-top: 8px; }
        .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .paid-badge { display: inline-flex; align-items: center; gap: 5px; background: rgba(212,175,55,0.1); border: 1px solid rgba(212,175,55,0.3); color: #d4af37; padding: 3px 10px; border-radius: 999px; font-size: 11px; font-weight: 600; }
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 20px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: -40, marginBottom: 28, flexWrap: "wrap", gap: 14 }}>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 16 }}>
              ...
              <div style={{ paddingBottom: 4 }}>
                ...
                <p style={{ color: "#555", fontSize: 13, margin: "0 0 8px" }}>
                  {artist.genre || artist.creative_field || "Creative"}{artist.location ? ` · ${artist.location}` : ""}
                </p>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <button className="ap-book-btn" onClick={() => setShowBooking(true)}>Book Now</button>
            </div>
          </div>

          {/* STATS */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 28 }}>
            <div className="ap-stat"><p className="ap-stat-num">{tracks.length}</p><p className="ap-stat-label">Works</p></div>
            <div className="ap-stat"><p className="ap-stat-num">{likeCount}</p><p className="ap-stat-label">Likes</p></div>
          ...
          <div style={{ marginBottom: 80 }}>
            <h2 className="ap-syne" style={{ fontSize: 19, fontWeight: 700, marginBottom: 16 }}>
              Work {tracks.length > 0 ? `(${tracks.length})` : ""}
            </h2>
            {tracks.length === 0 ? (
                ...
                {tracks.map((track: any) => (
                  <div key={track.id} className="ap-track">
                    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px" }}>
                      <img src={track.image_url || "https://images.unsplash.com/photo-1511379938547-c1f69419868d?w=200"} alt={track.title} style={{ width: 50, height: 50, borderRadius: 10, objectFit: "cover", flexShrink: 0 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ fontSize: 12, color: "#555", margin: "2px 0 0", textTransform: "uppercase", letterSpacing: "0.06em" }}>{track.genre || "Creative Work"}</p>
                      </div>
          ...
          <div className="modal-box">
            ...
            {submitted ? (
              <div style={{ textAlign: "center", padding: "16px 0" }}>
                ...
                <button className="btn-cancel" onClick={() => { setShowBooking(false); setSubmitted(false) }}>Close</button>
              </div>
            ) : (
              <>
                <h3 className="ap-syne" style={{ fontSize: 20, fontWeight: 700, marginBottom: 6 }}>Book {artist.name}</h3>
                <p style={{ color: "#555", fontSize: 13, marginBottom: 22 }}>Fill in your event details and we'll send your request.</p>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                  ...
                  <div>
                    <label className="modal-label">Total Budget (optional)</label>
                  ...
                  <div>
                    ...
                    <textarea className="modal-input" placeholder="Tell the creative about your event..." rows={3} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} style={{ resize: "vertical" }} />
                  </div>

                  {/* DEPOSIT SECTION */}
                  <div style={{ background: "#0a0a0a", border: "1px solid #1f1f1f", borderRadius: 12, padding: "16px" }}>
                    <label className="modal-label" style={{ marginBottom: 8 }}>
                      Deposit Amount (₦) — optional but recommended
                    </label>
                    <input
                    ...
                    <p style={{ fontSize: 11, color: "#444", margin: "6px 0 0", lineHeight: 1.5 }}>
                      Paying a deposit secures the booking and shows you're serious. The creative will be notified of the payment.
                    </p>
                  </div>

                  {/* PAY BUTTON */}
                  {depositAmount && parseInt(depositAmount) >= 100 ? (
                    <button className="btn-pay" onClick={handlePayAndBook} disabled={loading}>
                      {loading ? (
                        <><div style={{ width: 16, height: 16, border: "2px solid rgba(0,0,0,0.3)", borderTopColor: "#000", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />Processing...</>
                      ) : (
                        <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>Pay ₦{parseInt(depositAmount).toLocaleString()} & Send Request</>
                      )}
                    </button>
                  ) : (
                    <button className="btn-pay" onClick={handleBookingFree} disabled={loading}>
                      {loading ? "Sending..." : "Send Booking Request"}
                    </button>
                  )}

                  {depositAmount && parseInt(depositAmount) >= 100 && (
                    <button className="btn-free" onClick={handleBookingFree} disabled={loading}>
                      Send without deposit
                    </button>
                  )}

                  <button className="btn-cancel" onClick={() => setShowBooking(false)}>Cancel</button>
